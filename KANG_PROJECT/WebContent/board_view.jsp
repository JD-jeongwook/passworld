...
<script src="js_file/board_view.js"></script>
<title>게시글 보기</title>
<script>
...
<body>
	<div id='contents'>
		...
		<section>
			...
			<hr>
				<footer id='move_list'>
					<a href="user_board.do?curPage=${ returnPage }"><input type='button' value='목록' class='list two'></a>
				...
				<form id="recommend_form">
					<input type='hidden' name="recommend" value="${ dto.recommend }">
					<input type="hidden" name="no" value='${ dto.no }'>
					<c:if test="${ sessionScope.id ne null }">
						<input type="button" id="like_button" onclick="return like()">
					</c:if>
					<c:if test="${ sessionScope.id eq null }">
						<input type="button" id="like_button" onclick="needLogin()">
					</c:if>
					<ul>
						<li><label for="like_button" id='heart'><i class="fas fa-heart"></i></label></li>
						<li>${ dto.recommend }</li>
					</ul>
				</form>
			<hr>
		...
		<footer id='comment'>
			...
			<c:forEach var="cList" items="${ list }">
				...
				<br> 
					...
					<c:choose>
					<c:when test="${ ( cdto.admin eq 1) or (sessionScope.id eq cList.id) }">
						<form method='post' action='comment_modify.do' name='comment_form'>
							...
							<a href="comment_delete.do?no=${ cList.no }&view_no=${ dto.no }&returnPage=${ returnPage }"><input type='button' value='삭제' class='select_button delete_button'></a>
						</form>
					</c:when>
