...
<body>
	...
	<section>
		<div id='head'>
			<p> <i class="fas fa-clipboard-list"></i> 자유게시판</p>
		</div>
		<nav>
			<%-- 로그인창 --%>
        <c:choose>
        	<c:when test="${ empty sessionScope.id }">
		        <article id="logout">
		            <form method="post" action="login_search_board.do">
	    	...
	    	<c:otherwise>
	        	<article id="login">
		            ...
		            <c:if test="${ dto.admin eq 1 }">
		           	...
		           	<a href="logOut_search_board.do?search_select=${ select }&search_keyword=${ keyword }" id="logout_box">로그아웃</a>
		            <%--<form method='post' action='logOut_search_board.do'>
		            	<input type='hidden' name='search_select' value="${ select }">
						<input type='hidden' name='search_keyword' value="${ keyword }">
		                <input type="submit" value="로그아웃" id='logout_box'>
		            </form>--%>
	       		 </article>
       		</c:otherwise>
        </c:choose>
		...
		<article>
			<table>
				...
				<c:forEach var="dto" items="${ list }">
					<tr>
						<td>${ dto.no }</td>
						<td><a href="search_view.do?no=${ dto.no }&returnPage=${ curPage }&select=${ select }&keyword=${ keyword }" <c:if test="${ dto.admin eq 1 }"> style="color:red"</c:if>>${ dto.title }
			...
			<c:forEach var="i" begin="0" end="${ maxPage }">
				<c:if test="${ curPage eq i }"><span id="select_page" class="none_select_page">${ i+1 }</span></c:if>
				<c:if test="${ curPage ne i }">
					<form method='post' action='search_board.do' id="ib">
						<input type='hidden' name='curPage' value="${ i }">
						<input type='hidden' name='search_select' value="${ select }">
						<input type='hidden' name='search_keyword' value="${ keyword }">
						<input type='submit' value="${ i+1 }" id="none_select_page" class="none_select_page">
					</form>
				</c:if>
			</c:forEach>
			</div>
			...
			<div id="searching">
				<form method='post' action='search_board.do' onsubmit="return wherego(this)">
					<select name='search_select' id='search_list'>
						...
						<option value='titleandcontents'<c:if test="${ select eq 'titleandcontents' }">selected</c:if>>제목 + 내용</option>
						<option value='writer'<c:if test="${ select eq 'writer' }">selected</c:if>>작성자</option>
